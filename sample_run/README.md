...
The default command (no `--investigate`) queues **16** accounts instead of 9 and costs
about a cent; the agent is more cautious than the rules and moves several accounts from
...
|---|---|
| Accounts scored | 300 |
| Queued to reps | **9** (7 more held back as a control group) |
| Sent for data refresh | 20 |
| Marketing nurture / left alone | 55 / 209 |
| Model | `claude-haiku-4-5-20251001`, live — `runs.jsonl` records `"mocked": false` |
| Model calls | 63 — 9 briefs, 54 investigation turns |
| **Measured cost** | **$0.1402**, of which **$0.1294 is the investigator** |
...
The cost split is worth noticing: the boundary agent is **92% of the model spend for 6.7%
of the accounts**. That is the argument for keeping it off by default and pointed only at
